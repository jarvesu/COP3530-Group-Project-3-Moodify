...
int main()
{
    ...
    while(runApplication)
    {
        ...
        auto minDistanceIDs = calcHeap(songs, criteria, numSongs);
...
//inserts a single value into a specified heap at the proper position
void heapInsert(pair<float, string>* heap, pair<float, string> toInsert, int& heapSize)
{
    ...
    //object is larger than  largest object, skip it.
    if((heapSize > 2 && toInsert.first > heap[heapSize-1].first || toInsert.first > heap[heapSize - 2].first) || (heapSize == 1 && toInsert.first > heap[heapSize-1].first))
    {
        return;
    }
    //while not in position, shift around
    //if value tries to shift OOB, drop the value from importance
    while(parent >= 0 && heap[parent].first > heap[child].first)
    {
